  ...
  const sys = {
    ...
    checkNodeModules() {
      ...
      if (!fs.existsSync(nodeModulesPath) || !fs.existsSync(path.resolve(nodeModulesPath, 'cheerio/'))) {
        ...
        exec('npm install', {cwd: pRoot, maxBuffer: 1024 * 1024 * 5}, (error, stdout, stderr) => {
          if (error) {
    ...
    startWebpackBuild(isRelease) {
      ...
      exec(`"${wpPath}"` + (isRelease ? ' --env.release' : ''), {cwd: pRoot, maxBuffer: 1024 * 1024 * 5}, (error, stdout, stderr) => {
        if (error) {
